...
export function scoreSide(entries, tickets) {
  const lines = entries
    ...
    .map(entry => {
      ...
      const value = signedValue(unitValue(unit), sign);
      return { name: unit.name, image: unit.image, rarity: unit.rarity, demand: unit.demand, status: unit.status, sign, quantity: entry.quantity, value, total: value * entry.quantity };
    });
