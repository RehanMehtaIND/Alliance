...
// Some units are valued by serial number: each range in unit.serials has its own value,
// and a null value means those serials aren't in circulation. "to": null is open-ended.
const serialRange = (unit, serial) => unit.serials.find(range => serial >= range.from && (range.to == null || serial <= range.to));
const serialLabel = range => range.to == null ? `#${range.from}+` : range.from === range.to ? `#${range.from}` : `#${range.from}–${range.to}`;
const serialUnitNames = units.filter(unit => unit.serials).map(unit => unit.name);

// A sign from a staff member or partner adds its boost percent to the unit's value.
export const NO_SIGN = 'None';
export const signs = JSON.parse(readFileSync(join(process.cwd(), 'signs.json'), 'utf8'));
const signByName = new Map(signs.map(sign => [sign.name, sign]));
export const signedValue = (value, sign) => Math.round(value * (1 + (sign?.boost || 0) / 100) * 100) / 100;

export const unitListText = units.map(unit => `- ${unit.name} (${unit.rarity})`).join('\n');
export const SLANG_RULE = '- Players use slang and shorthand: "cam" = Camera Man, "tv" = TV Man, "speaker" = Speaker Man, "dj" = DJ Speaker Man, "ptv" or "party titan tv" = Party Titan TV Man, "engi" = Engineer, "tsm" = Titan Speaker Man, "uts"/"upts"/"ts 2.0"/"tsm 2.0" = Titan Speaker Man 2.0, "tcm" = Titan Camera Man, "utc"/"uptc" = Upgraded Titan Camera Man, "builder"/"constructor" = Builder Camera Man, "10m speaker"/"10msm" = 10M Speaker Man. Map each mention to the closest unit on the list and record the guess in assumptions when it is not obvious.';

export const SERIAL_RULE = `- These units are valued by serial number: ${serialUnitNames.join(', ')}. Put the serial in serial ("10m speaker #45", "serial 45", "s45" → 45), or 0 when none is given and for every other unit. List copies with different serials separately.`;

export const SIGN_RULE = `- A unit can carry a sign, which raises its value ("petcch signed tcm", "tcm with remilia sign", "signed by niahuser"). Put the signer in sign, matching the closest name below (other names they go by are in parentheses), or "${NO_SIGN}" when the unit isn't signed. Every unit in a stack shares one sign; list differently signed copies of a unit separately. A sign by someone not on this list goes in unrecognized and the unit counts as unsigned.
Signs:
${signs.map(sign => `- ${sign.name}${sign.aliases ? ` (${sign.aliases.join(', ')})` : ''}`).join('\n')}`;
...
export const sideSchema = {
  ...
  items: {
    ...
    properties: {
      ...
      sign: { type: 'string', enum: [NO_SIGN, ...signs.map(sign => sign.name)] },
      serial: { type: 'integer' },
    },
    required: ['name', 'quantity', 'sign', 'serial'],
...
export function scoreSide(entries, tickets) {
  const lines = entries
    ...
    .map(entry => {
      ...
      const found = signByName.get(entry.sign);
      const sign = found && { name: found.name, boost: found.boost };
      const line = { name: unit.name, image: unit.image, rarity: unit.rarity, demand: unit.demand, status: unit.status, sign, quantity: entry.quantity };
      let base = unitValue(unit);
      if (unit.serials) {
        const serial = Number.isInteger(entry.serial) && entry.serial > 0 ? entry.serial : null;
        const range = serial && serialRange(unit, serial);
        line.serial = serial;
        if (range) line.serialRange = { label: serialLabel(range), note: range.note };
        base = range?.value ?? null;
        // Unpriced lines count as 0 and carry the reason, so no verdict is given for them.
        if (base == null) line.unpriced = range ? `${unit.name} #${serial} isn't in circulation, so it has no listed value.` : `${unit.name}'s value depends on its serial number. Include it, like "${unit.name} #120", to value it.`;
      }
      line.value = base == null ? 0 : signedValue(base, sign);
      line.total = line.value * entry.quantity;
      return line;
    });
...
// Reasons some lines in these sides couldn't be valued.
export const unpricedNotes = (...sides) => [...new Set(sides.flatMap(side => side.lines.map(line => line.unpriced).filter(Boolean)))];
